...
void Main()
{
	...
	var operatorModules = DB.OperatorModules
		...
		.Select((v, i) => HighlightIf(v.Total > 1, new
		{
			...
			Operator    = VerticalRun(v.Operators.Select(static v => new WikiHyperlinq(v.Operator))),
	...
	var unknownOperators = operators.Select(static op => op.Name)
		.Except(DB.OperatorModules.Select(static om => om.Operator), StringComparer.OrdinalIgnoreCase)
...
internal record OperatorModule(string Operator, string Class, int Stars, string Module, string Stage, string E2Materials, bool Paradox);
...
class OperatorModules : Parsable<OperatorModule>
{
	...
	protected override string Regex { get; } = $@"^(?<{Operator}>[^\t]+)\t+(?<{Class}>[^\t]+)\t+(?<{Stars}>[1-6])\t+(?<{Stage}>[^\t]+)\t+(?<{Module}>[^\t]+)\t+(?<{E2Materials}>[^\t]+)(\t+(?<{Paradox}>[^\t]+))?$";
